...
bool Sphere::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const
{
  ...
  float B = 2.0 * ( 
            (r.origin.x-center.x)*r.direction.x 
          + (r.origin.y-center.y)*r.direction.y
          + (r.origin.z-center.z)*r.direction.z );

  float C = (r.origin.x-center.x)*(r.origin.x-center.x)
          + (r.origin.y-center.y)*(r.origin.y-center.y) 
          + (r.origin.z-center.z)*(r.origin.z-center.z) 
          - radius*radius;
  ...
  float dist = 0;
  // wow wow is that reasonable (strict equality between floats)?
  if ( delta == 0.0 )
      dist = -B/(2.0*A);
  else if ( delta > 0.0 )
      dist = (-B-sqrt( delta ))/(2.0*A);
  else assert( "should not go" == "there" );
  ...
  hit.dist = dist;
  float3 normal; 
  normal.x = r.origin.x + dist*r.direction.x - center.x;
  normal.y = r.origin.y + dist*r.direction.y - center.y;
  normal.z = r.origin.z + dist*r.direction.z - center.z;
